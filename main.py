...
# Now we get to use a trick like a dataclass - using the type hints to declare the type
# We say what Python type we want and the orm decides how best to achieve this
...
with (orm.Session(engine) as session):
    ...
    for album in genre_query:
        print(f'{album} by {album.artist}, {album.genres}')

    print('\nAnd now pointlessly filtering using Python rather than the orm\n')

    for album in [a for a in session.query(Album) if genre2 in a.genres]:
        print(f'{album} by {album.artist}, {album.genres}')
